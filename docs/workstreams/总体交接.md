...
本文档把已确认的 Lithe 产品设计拆成八个 tracer bullet 垂直工作流，并给出直接
...
flowchart LR
    ...
    WS02 --> WS07["WS07 文件浏览与编辑"]
    WS07 --> WS08["WS08 Git 变更审阅"]
```
...
- 标签/拆分布局引擎、拖拽命中与尺寸计算；
- 代码编辑器、富文本编辑器、语法高亮、光标与撤销栈；
- Git Diff 解析、hunk/行级对齐和 Diff 渲染；
- 大型文件树的虚拟滚动、键盘导航和无障碍交互。
...
形成 ADR；未经该确认不得转为自行实现通用引擎。当前文件编辑范围是代码/文本
文件，不因本原则自动扩大为富文本功能；未来确需富文本时同样必须采用成熟编辑器
框架。
...
- 边界：包含三平台本机 PTY、Shell 检测、递归布局树、标签组、移动、拆分、
  最大化、布局持久化和退出清理；不包含 Agent、文件编辑与 Git Diff 正文。
- 直接依赖：WS01。
...
- `docs/adr/0018-前台可见才更新任务已读时间.md`
...
- 目标：用户通过工作区文件树打开已有文件，在共享文档模型中编辑、处理外部冲突
  并显式保存。
- 一句话描述：把只读文件树点击贯穿到编辑面板、共享内存文档和受控磁盘写入。
- 边界：包含右侧“文件”树、工作区/隐藏/符号链接边界、编辑器、多视图共享和退出
  保存确认；不包含新建、重命名、移动、删除及 Git Diff。
- 直接依赖：WS02。

### 交付物

- 工作区边界内的文件枚举、读取、监听与保存 main 服务及窄 IPC。
- 右侧侧边栏状态和只读文件树。
- 文件编辑面板、共享文档 store、显式保存与冲突模型。
- 关闭最后视图和应用退出时的聚合保存确认。

### 完成条件

- `.git` 和操作系统隐藏文件始终隐藏；忽略文件按工作区设置切换。
- 外部符号链接目标不可遍历或编辑。
- 每次点击都在当前活动标签组创建新面板，同文件视图共享文档。
- 干净文档遇外部修改自动刷新；脏文档进入冲突状态且不静默覆盖。
- 首版只能保存已有文件，文件树没有结构写操作。
- 文件边界、冲突、重复视图和真实保存路径均有测试与 Electron E2E。

### 建议 Skill

- 实现：`/code-spec`；路径与文档状态机使用 `/tdd`；文件树、编辑、冲突和退出确认
  使用 `/e2e`。
- UI：`/design-philosophy`，必要时 `/frontend-design`。
- 交付：`/verifying`，然后 `/code-review`。
...
- `docs/adr/0036-右侧侧边栏只承载文件导航树.md`
- `docs/adr/0037-文件编辑显式保存且多视图共享文档.md`
- `docs/adr/0040-文件树限制在工作区边界内.md`
- `docs/adr/0020-工作区直接承载唯一中间主界面.md`
- `docs/adr/0021-任务独立于Agent面板存在.md`

### 第一步行动

先调研成熟的代码编辑器与大型文件树框架，把编辑器文档 API 封装为 Lithe 的共享
文档模型；再固定 main 文件服务的工作区边界，实现“点击一个现有文件、编辑并
显式保存”的 tracer bullet。不得自行实现编辑器核心、撤销栈或文件树虚拟化。

## WS08-Git变更审阅

### 定义

- 目标：用户通过变更树分别审阅 staged、unstaged 和 untracked working-tree Diff。
- 一句话描述：把 Git working tree 状态贯穿到右侧变更树和中间只读 Diff 面板。
- 边界：包含右侧“变更”树、只读 Diff 面板、同文件多分组和文件变化刷新；不包含
  stage、discard、commit、checkout、冲突解决或分支比较。
- 直接依赖：WS07。

### 交付物

- main 中只读 Git status/diff 服务与窄 IPC。
- staged、unstaged、untracked 三组变更树。
- 当前标签组中新建的 Git Diff 面板和刷新机制。
- 无 Git 工作区隐藏“变更”标签的 UI。

### 完成条件

- staged 使用 HEAD 对 index，unstaged 使用 index 对 working tree，untracked
  使用空文件对实际文件。
- 同一文件可同时出现在 staged 与 unstaged，不错误合并。
- 每次点击都创建新 Diff 面板，不复用已有面板。
- 所有 Git 写操作均不存在；不推断默认分支、远端或 merge-base。
- 临时 Git 仓库覆盖三类差异、同文件双状态、无 Git 目录和实时刷新。

### 建议 Skill

- 实现：`/code-spec`；Git 差异语义和解析使用 `/tdd`；变更树与 Diff 面板路径
  使用 `/e2e`。
- UI：`/design-philosophy`。
- 交付：`/verifying`，然后 `/code-review`。
...
- `docs/adr/0036-右侧侧边栏只承载文件导航树.md`
- `docs/adr/0038-首版GitDiff只提供只读审阅.md`
- `docs/adr/0020-工作区直接承载唯一中间主界面.md`

### 第一步行动

先调研成熟的 Git 调用封装与 Diff 渲染框架；用临时 Git 仓库写出 staged、
unstaged、untracked 的可观察契约测试，再通过窄 Adapter 接到右侧变更树和只读
Diff 面板。不得自行实现通用 patch 解析、hunk 对齐或行级 Diff 渲染算法。
