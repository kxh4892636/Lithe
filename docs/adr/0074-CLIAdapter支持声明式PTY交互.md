...
Kimi Code 内置 Adapter 的 `fork` 先以源会话 ID 恢复会话，再在终端就绪后输入
`/fork`。Lithe 将 Kimi Code 在派生会话创建后触发新的 `SessionStart` Hook
视为该集成的既定契约；Hook 提交的新 `session_id` 直接绑定新任务。这里不增加
版本探测、第二套会话发现机制或对提供方历史目录的扫描。
